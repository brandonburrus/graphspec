...
types or relations outside this list — `npx graph-spec-cli validate` only knows these.
...
repo's own dogfood bundle carries them, and `npx graph-spec-cli index`/`npx graph-spec-cli query` render `title`
...
## Validation model (what `npx graph-spec-cli validate` actually checks)
